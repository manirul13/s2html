...
pevent_t *pstate_reserve_keyword_handler(FILE *fd, int ch)
{
	...
	default:
		...
		word_idx = 0;
		fseek(fd, -1L, SEEK_CUR); // unread last character
		set_parser_event(PSTATE_IDLE, PSTATE_RESERVE_KEYWORD);
		return &pevent_data;
	}
	return NULL;
}
...
pevent_t *pstate_numeric_constant_handler(FILE *fd, int ch)
{
	...
	default:
		...
		set_parser_event(PSTATE_IDLE, PSTATE_NUMERIC_CONSTANT);
		return &pevent_data;
	}
	return NULL;
}
